...
const THROW_MS = 300;
/*
 * 먹은 패가 더미로 들어가는 건 "무엇을 먹었는지" 읽는 순간이다.
 * 다만 이건 한 판에 스무 번 넘게 보는 연출이라, 읽을 만큼만 두고 걷는다.
 * 중간에 한 번 크게 키워 보여주므로 비행이 짧아도 무엇인지는 남는다.
 * 여러 장이 한꺼번에 날면 겹쳐서 안 보이므로 한 장씩 시차를 둔다.
 */
const SWEEP_MS = 660;
const SWEEP_WAIT = 280;
/*
 * 한 장이 다 날아간 뒤에 다음 장이 뜨면 뚝뚝 끊겨 보인다.
 * 비행시간보다 훨씬 짧게 띄워서 줄줄이 이어지는 한 줄기로 만든다.
 */
const SWEEP_STAGGER = 110;
